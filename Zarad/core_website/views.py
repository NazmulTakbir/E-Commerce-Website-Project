...
def topCategories(request):
    ...
    query = """SELECT CATEGORY_ID, Y.CATEGORY_NAME, Y.PICTURE FROM (SELECT B.CATEGORY_ID FROM
...
def getProductDetails(productIDs):
    ...
    with connections['oracle'].cursor() as cursor:
        ...
        query = """SELECT W.PRODUCT_ID, W.PNAME, W.SELLER_ID, W.SNAME, W.PRICE, W.DISCOUNT, W.RATING, PP.PICTURE, PPP.PICTURE
