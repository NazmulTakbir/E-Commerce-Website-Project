...
def topCategories(request):
    ...
    query = """SELECT CATEGORY_ID, B.CATEGORY_NAME, B.PICTURE FROM
              ...
              (SELECT CATEGORY_ID FROM PRODUCT WHERE PRODUCT_ID = O.PRODUCT_ID)CATEGORY_ID ,
              COUNT(O.PRODUCT_ID)NUM FROM CUSTOMER_ORDER C, ORDERED_ITEMS  O, PURCHASE_ORDER P WHERE C.ORDER_ID = O.ORDER_ID
